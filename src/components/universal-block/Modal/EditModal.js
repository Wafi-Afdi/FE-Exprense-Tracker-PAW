...
function EditCreatePopupModal({
  ...
  return (
    <div className="fixed inset-0 w-full h-full bg-slate-300 z-[120] bg-opacity-25 backdrop-blur-sm text-xl flex justify-center items-center">
      <div className="flex flex-col p-5 bg-white min-w-[600px] border-2 border-black rounded-lg">
        <div className="flex flex-row justify-between items-center">
          <div className="text-2xl font-bold">{isEdit ? "Edit" : "Buat"}</div>
          <button
            id="close"
            name="close"
        ...
        <form className="flex flex-col gap-4 mt-3" onSubmit={onSubmit}>
          ...
          <div className="w-full flex flex-wrap gap-2 mt-2">
            <button
              className="p-2 rounded-md bg-green-600 text-gray-100 text-md"
              name="save"
              id="save"
              type="submit"
            >
            ...
            {isEdit && (
              <button
                className="p-2 rounded-md bg-red-600 text-gray-100 text-md"
                name="delete"
                id="delete"
                type="button"
                onClick={(e) => onDelete()}
